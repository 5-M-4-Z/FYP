...
# Specify the full path to the notepad (.txt) file
file_path = r"C:\Users\HU\OneDrive - Habib University\Desktop\4-patch\generated_points.txt"
...
for line in file:
	...
	parameter_values = list(map(float, line.split()))
	
	hole_xx = parameter_values[0]
	hole_yy = parameter_values[1]
	truncation_x = parameter_values[2]
	truncation_y = parameter_values[3]
	patch_xsize_num = parameter_values[4]
	patch_ysize_num = parameter_values[5]
	patch_x_num = parameter_values[6]
	patch_y_num = parameter_values[7]

	name_of_file = (
		"hole_x=" + str(hole_xx) + ", " +
		"hole_y=" + str(hole_yy) + ", " +
		"trunc_x=" + str(truncation_x) + ", " +
		"trunc_y=" + str(truncation_y) + ", " +
		"patch_xsize=" + str(patch_xsize_num) + ", " +
		"patch_ysize=" + str(patch_ysize_num) + ", " +
		"patch_x=" + str(patch_x_num) + ", " +
		"patch_y=" + str(patch_y_num)
	)
	...
	oEditor.CreatePolyline(
		[
			...
			[
				...
				["NAME:PLPoint", "X:=", "%fmm" % (x1 + truncation_x), "Y:=", "%fmm" % y1, "Z:=", "%fmm" % z1],  
				["NAME:PLPoint", "X:=", "%fmm" % x1, "Y:=", "%fmm" % (y1 - truncation_y), "Z:=", "%fmm" % z1]   
			],
			[
	...
	oEditor.CreatePolyline(
		[
			...
			[
				...
				["NAME:PLPoint", "X:=", "%fmm" % (x2 - truncation_x), "Y:=", "%fmm" % y2, "Z:=", "%fmm" % z2],
				["NAME:PLPoint", "X:=", "%fmm" % x2, "Y:=", "%fmm" % (y2 + truncation_y ), "Z:=", "%fmm" % z2]
			],
			[
				"NAME:PolylineSegments",
				["NAME:PLSegment", "SegmentType:=", "Line", "StartIndex:=", 0, "NoOfPoints:=", 2],
	...
	oProject.SaveAs("C:\\Users\\HU\\OneDrive - Habib University\\Desktop\\4-patch\\Temp\\temp.aedt", True)
	oDesign.AnalyzeAll()
	...
	oModule.ExportToFile("S Parameter Plot 1", "C:/Users/HU/OneDrive - Habib University/Desktop/4-patch/S11_Data_Sets/S-11 Plot -- " + name_of_file + ".csv", False)
	oModule.ExportToFile("Axial Ratio Plot 1", "C:/Users/HU/OneDrive - Habib University/Desktop/4-patch/Axial_Ratio_Data_Sets/Axial Ratio Plot -- " + name_of_file + ".csv", False)
